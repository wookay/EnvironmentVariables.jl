...
const STDLIB_Test_PATCHES = Vector{Patch}([
    # v1.13
    Patch(v"1.13.0-DEV.1075",
        "0b39226110", # Add test verbose env var and show when starting/finishing testsets
        Path"stdlib/Test/src/Test.jl",
        EnvKey"JULIA_TEST_VERBOSE" => "const VERBOSE_TESTSETS"
    ),
    Patch(v"1.13.0-DEV.259",
        "d9711317e8", # CI: Store Passes in results.json + various improvements
        Path"stdlib/Test/src/Test.jl",
        EnvKey"JULIA_TEST_RECORD_PASSES" => "const TEST_RECORD_PASSES"
    ),

    # v1.9
    Patch(v"1.9.0-DEV.623",
        "88def1afe1", # Test: Add fail-fast mechanism
        Path"stdlib/Test/src/Test.jl",
...
const STDLIB_REPL_PATCHES = Vector{Patch}([
    # v1.11
    Patch(v"1.11.0-DEV.1261",
        "893e720fe8", # REPLCompletions: async cache PATH scan
        Path"stdlib/REPL/src/REPLCompletions.jl",
        EnvKey"PATH" => "function cache_PATH()"
    ),

    # v0.7
    Patch(v"0.7.0-DEV.3817",
        "13b9902b81", # move TerminalMenus.jl into a submodule in stdlib/REPL
    ...
    Patch(v"0.7.0-DEV.3498",
        "73c989ba51", # move REPL to stdlib
