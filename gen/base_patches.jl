...
const BASE_PATCHES = Vector{Patch}([
    ...
    Patch(v"1.14.0-DEV.2425",
        "e319a303a7", # precompilation: coordinate number of workers and imaging threads via a jobserver
    ...
    Patch(v"1.14.0-DEV.2243",
        "f022e008b5", # precompilepkgs: add more timing info in timing & verbose mode
        Path"base/loading.jl",
        EnvKey"JULIA_PRECOMP_REPORT_TIMING" => signature_base_loading_include_package_for_output
    ),
    Patch(v"1.14.0-DEV.2014",
        "9b03f4ae1b", # Redact sensitive env vars when printing cmd errors
        Path"base/cmd.jl",
        EnvKey"JULIA_SHOW_ENV" => "function get_show_env_mode(io::IO)"
    ),
    Patch(v"1.14.0-DEV.1963",
        "cb2e1ecf24", # precompilepkgs: optionally run precompilation in background task with keyboard controls
        Path"base/precompilation.jl",
        EnvKey"TERM" => signature_base_precompilation_monitor_background_precompile
    ),

    # v1.13
    Patch(v"1.13.0-DEV.1217",
        "c4683c41a8", # Enable verbose debugging when github actions etc. requests it
        Path"base/client.jl",
    ...
    Patch(v"1.13.0-DEV.840",
        "72e2c45554", # Make current_terminfo a OncePerProcess
        Path"base/terminfo.jl",
        EnvKey"TERM" => "const current_terminfo"
    ),

    # v1.12
    Patch(v"1.12.0-DEV.1244",
        "a06a80162b", # Add filesystem func to transform a path to a URI
        Path"base/path.jl",
        EnvKey"WSL_DISTRO_NAME" => "function uripath(path::String)"
    ),
    Patch(v"1.12.0-DEV.912",
        "0bb650ccbf", # [REPL] improve quality of precompile script
        Path"base/client.jl",
        EnvKey"TERM" => "function run_std_repl(REPL::Module, quiet::Bool, banner::Symbol, history_file::Bool)"
    ),
    Patch(v"1.12.0-DEV.612",
        "0bf392e671", # Enable support for `NO_COLOR` and `FORCE_COLOR` environment variables
    ...
    Patch(v"1.12.0-DEV.505",
        "3638c84f45", # move ConsoleLogger into Base
        Path"base/logging/logging.jl",
        EnvKey"JULIA_DEBUG" => "global Base.@constprop :none function env_override_minlevel(group, _module)"
    ),
    Patch(v"1.12.0-DEV.151",
        "78351b5c47", # Use Base parallel precompilation to build stdlibs
        Path"base/util.jl",
        EnvKey"JULIA_TESTS" => signature_base_util_runtests
    ),
    Patch(v"1.12.0-DEV.128",
        "67451604cf", # Move parallel precompilation to Base
        Path"base/precompilation.jl",
    ...
    # v1.11
    Patch(v"1.11.0-DEV.897",
        "9729f31218", # Tweak test LOAD_PATH and fix loading test
        Path"base/util.jl",
        EnvKey"JULIA_LOAD_PATH" => signature_base_util_runtests
    ),
    Patch(v"1.11.0-DEV.504",
        "e7290dc1fc", # Add environment variable to force the use of the fallback repl
        Path"base/client.jl",
        EnvKey"JULIA_FALLBACK_REPL" => "function run_main_repl(interactive::Bool, quiet::Bool, banner::Symbol, history_file::Bool)"
    ),
    Patch(v"1.11.0-DEV.307",
        "2690ca8cc6", # Terminfo parser
        Path"base/terminfo.jl",
    ...
    # v1.10
    Patch(v"1.10.0-DEV.1294",
        "e4924c51de", # add devdocs how to profile package precompilation with tracy
        Path"base/Base.jl",
        EnvKey"JULIA_WAIT_FOR_TRACY" => "function __init__()"
    ),
    Patch(v"1.10.0-DEV.204",
        "a2db90fe8d", # Implement support for object caching through pkgimages
        Path"base/loading.jl",
    ...
    # v1.8
    Patch(v"1.8.0-DEV.1185",
        "f61eb87cd8", # `Base.runtests`: unset `JULIA_PROJECT` and `JULIA_LOAD_PATH`
        Path"base/util.jl",
        EnvKey"JULIA_PROJECT" => signature_base_util_runtests
    ),
    Patch(v"1.8.0-DEV.590",
        "2c9e051c46", # `Base.runtests`: set a temporary Pkg depot path
        Path"base/util.jl",
        EnvKey"JULIA_DEPOT_PATH" => signature_base_util_runtests
    ),

    # v1.6
    Patch(v"1.6.0-DEV.1167",
        "58605d3702", # Make it possible to override MAX_NUM_PRECOMPILE_FILES using ENV
        Path"base/Base.jl",
        EnvKey"JULIA_MAX_NUM_PRECOMPILE_FILES" => "function __init__()"
    ),
    Patch(v"1.6.0-DEV.354",
        "30b09dfd2b", # [WIP] include ClearStacktrace.jl in Base
        Path"base/errorshow.jl",
    ...
    # v0.7
    Patch(v"0.7.0-beta.280",
        "9050651b24", # Rename Sys.CPU_CORES => Sys.CPU_THREADS
    ...
    Patch(v"0.7.0-beta.90",
        "2549e227cf", # Pkg + code loading: home project & active project concepts
        Path"base/initdefs.jl",
        EnvKey"JULIA_PROJECT" => "function init_active_project()"
    ),
    Patch(v"0.7.0-DEV.5168",
        "a348496165", # Add `Sys.which(program_name::AbstractString)`
        Path"base/sysinfo.jl",
        EnvKey"PATH" => "module Sys function which(program_name::String)"
    ),
    Patch(v"0.7.0-DEV.4898",
        "50413a46a0", # change default LOAD_PATH and DEPOT_PATH
        Path"base/loading.jl",
    ...
    Patch(v"0.7.0-DEV.3483",
        "865c08e0ce", # Pkg3-style code loading, DEPOT_PATH, tests
        Path"base/initdefs.jl",
        EnvKey"JULIA_DEPOT_PATH" => "function init_depot_path()"
    ),
    Patch(v"0.7.0-DEV.3182",
        "9d2bc2cbd9", # some logging things
        Path"base/client.jl",
        EnvKey"JULIA_DEBUG_COLOR" => "function debug_color()"
    ),

    # v0.6
    Patch(v"0.6.0-dev.1749",
        "dc7b4fbcf3", # Try make REPL error easier to visually parse
        Path"base/client.jl",
    ...
    Patch(v"0.6.0-dev.1545",
        "2fc856d739", # introduce JULIA_ERROR_COLOR to control how errors are printed
        Path"base/client.jl",
        EnvKey"JULIA_ERROR_COLOR" => "function error_color()"
    ),

    # v0.4
    Patch(v"0.4.0-dev+1914",
        "52fecaf2c6", # deprecate tty_size, replace with iosize & export it
        Path"base/stream.jl",
        EnvKey"LINES" => "displaysize()",
        EnvKey"COLUMNS" => "displaysize()"
    ),
    Patch(v"0.4.0-dev+1852",
        "e84d85dcac", # Fix #9103, implementing `cd -` in REPL shell mode
        Path"base/client.jl",
        EnvKey"OLDPWD" => "function repl_cmd(cmd::AbstractCmd, out)"
    ),
    Patch(v"0.4.0-dev+1734",
        "571a8c779e", # split client.jl into REPL-related pieces and basic initialization definitions
        Path"base/initdefs.jl",
        EnvKey"JULIA_LOAD_PATH" => "function init_load_path()"
    ),
    Patch(v"0.4.0-dev+1604",
        "6c5265316b", # `@threadcall`: ccall-like macro for doing work in another thread
        Path"base/threadcall.jl",
        EnvKey"UV_THREADPOOL_SIZE" => "const max_ccall_threads"
    ),
    Patch(v"0.4.0-dev+678",
        "12cc0d3d19", # Make info & warn colors configurable via ENV
        Path"base/client.jl",
    ...
    # v0.2
    Patch(v"0.2+215",
        "6f7f6a72ad", # add JULIA_INPUT_COLOR. closes #2441
        Path"base/client.jl",
        EnvKey"JULIA_INPUT_COLOR" => "function input_color()"
    ),
    Patch(v"0.2.0-rc2+39",
        "85d81b33e8", # `;cmd`: let JULIA_SHELL over-ride the value of SHELL
        Path"base/client.jl",
        EnvKey"JULIA_SHELL" => "function repl_cmd(cmd::AbstractCmd, out)"
    ),
    Patch(v"0.2.0-rc1+173",
        "37ae1e4abf", # run `;cmd` through a shell
        Path"base/client.jl",
    ...
    # v0.0
    Patch(v"0",
        "26c23f48bc", # don't use ANSWER_COLOR global variable
        Path"base/client.jl",
