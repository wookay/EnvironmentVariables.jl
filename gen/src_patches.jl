...
const SRC_PATCHES = Vector{Patch}([
    ...
    Patch(v"1.14.0-DEV.2582",
        "a6cf20fa5d", # Integrate MMTk binding into Julia build (src/gc-mmtk)
    ...
    Patch(v"1.14.0-DEV.2313",
        "acaea7226f", # Add dynamic ASAN detection for RTLD_DEEPBIND / `dlopen` workarounds
        Path"src/dlload.c",
    ...
    Patch(v"1.14.0-DEV.2037",
        "d5c76d6dcb", # Replace hand-maintained CPU tables with cpufeatures library
        Path"src/processor.cpp",
        EnvKey"JULIA_DEBUG" => "static bool cpufeatures_debug_enabled() JL_NOTSAFEPOINT"
    ),

    # v1.12
    Patch(v"1.12.0-DEV.1578",
        "f336314762", # Make heap size hint available as an env variable
        Path"src/gc-stock.c",
        EnvKey"JULIA_HEAP_SIZE_HINT" => "void jl_gc_init(void)"
    ),

    # v1.11
    Patch(v"1.11.0-DEV.1076",
        "bb2822275b", # better support threads in precompile and jl_task_wait_empty
        Path"src/scheduler.c",
        EnvKey"JULIA_THREAD_SLEEP_THRESHOLD" => "void jl_init_threadinginfra(void)"
    ),

    # v1.10
    Patch(v"1.10.0-DEV.1147",
        "abeecee71c", # Implement parallel marking
        Path"src/threading.c",
        EnvKey"JULIA_NUM_GC_THREADS" => "void jl_init_threading(void)"
    ),
    Patch(v"1.10.0-DEV.964",
        "8e0cba5663", # timing: add envvars for subsystem enable and verbose metadata
        Path"src/jlapi.c",
    ...
    Patch(v"1.10.0-DEV.716",
        "4e35f416bb", # Clean up timers and prints, link to JULIA_IMAGE_TIMINGS
        Path"src/aotcompile.cpp",
...
const CLI_PATCHES = Vector{Patch}([
    # v1.14
    Patch(v"1.14.0-DEV.2836",
        "efbb50fdff", # cli: add `JULIA_LOAD_CODEGEN_LIB` environment variable to disable `libjulia-codegen`
        Path"cli/loader_lib.c",
        EnvKey"JULIA_LOAD_CODEGEN_LIB" => "__attribute__((constructor)) void jl_load_libjulia_internal(void)"
    ),

    # v1.9
    Patch(v"1.9.0-DEV.1779",
        "eb708d62f8", # Probe and dlopen() the correct libstdc++
        Path"cli/loader_lib.c",
